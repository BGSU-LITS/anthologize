...
<html>
	<head>
		...
		<style type="text/css">
			body{ font-size:<?php echo $font_size; ?>pt; }
			h1{ font-size:2em; }
			h2{ font-size:1.8em; }
			h3{ font-size:1.6em; }
			h4{ font-size:1.4em; }
	...
	<body>
		...
		<p class="project-anthologizer">Anthologized by: <?php echo $project->anthologizer(); ?></p>
...
<?php foreach ($project->parts() as $part): ?>
	...
	<?php if (count($part->posts() > 0)): ?>
			<div class="post">
				...
				<?php if ($display_comments == true AND count($post->comments())): ?>
